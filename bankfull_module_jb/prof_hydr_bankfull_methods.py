...
def find_bankfull_M1(
    spline_results: list, directory_path: str, dist_pic: float, prominence: float
) -> tuple:
    """
    Trouve l'altitude maximale de débordement pour chaque profil et
    affiche les amplitudes positives pour chaque transect.

    :param spline_results: Liste de tuples (group_name, ref_altitude_smooth,
                        profondeur_hydraulique_smooth) représentant les résultats
                        du lissage de la courbe de profondeur hydraulique.
    :param directory_path: Chemin du répertoire pour sauvegarder le fichier de sortie.

    :return: Tuple contenant deux éléments
            - Liste de tuples contenant l'indice du profil et l'altitude maximale
            de débordement pour ce profil.
            - Dictionnaire contenant les amplitudes positives par transect.
    """
    altitude_max_amplitudes = []
    amplitudes_per_transect = {}
    ...
    for (
        ...
        peaks, _ = find_peaks(
            profondeur_hydraulique_smooth_array,
            distance=dist_pic,
...
def find_bankfull_M2(
    ...
    :param spline_results: Liste de tuples contenant les résultats du lissage de la
                        courbe de profondeur hydraulique.
    :param directory_path: Chemin du répertoire pour sauvegarder le fichier de sortie.
    ...
    # Appel à find_bankfull_M1 pour obtenir les altitudes maximales et les amplitudes
    _, amplitudes_per_transect = find_bankfull_M1(
        spline_results, directory_path, dist_pic, prominence
    )
    ...
    for idx, ref_altitude_smooth, profondeur_hydraulique_smooth in spline_results:
        ...
        maxima_smooth, _ = find_peaks(
            profondeur_hydraulique_smooth_np, distance=dist_pic, prominence=prominence
        )
        minima_smooth, _ = find_peaks(
            -profondeur_hydraulique_smooth_np, distance=dist_pic, prominence=prominence
        )
        ...
        if maxima_smooth.size > 0 and minima_smooth.size > 0:
            if idx == 0:
                ...
                if altitude_at_max_amplitude is not None:
                    ...
                    print(
                        f"Altitude de débordement pour le profil {idx}: {alti_bankfull}"
                    )
            else:
                if amplitudes_per_transect.get(idx):
                    prev_altitude = prev_bankfull[1]

                    # Trouver l'amplitude dont l'altitude est la plus proche
                    # de celle du profil précédent
