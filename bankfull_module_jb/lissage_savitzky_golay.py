...
def curve_Savitzky_Golay(curve_data, cross_section_data):
    ...
    for x_sec_id, group in grouped_data:
        ...
        smoothed_curve = savgol_filter(curve, window_length=5, polyorder=4)
        #print("smoothed_curve :", smoothed_curve) 

        # Détection des pics dans la courbe de courbure
        peaks, _ = find_peaks(smoothed_curve, distance=1, height=0.001, prominence=0.001)
        print("Indices des pics de courbure détéctés:", peaks)  # Afficher les pics détectés
        ...
        print("Indices des pics filtrés hors des zones en eau:", filtered_peaks)  # Afficher les pics filtrés
        ...
        mean_altitude_before = river_banks[river_banks['Distance'] < river_center.iloc[0]['Distance']]['POINT_Z'].mean()
        mean_altitude_after = river_banks[river_banks['Distance'] > river_center.iloc[0]['Distance']]['POINT_Z'].mean()
        ...
        if min_bank == 'gauche':
            bank_indices = river_banks[river_banks['Distance'] < river_center.iloc[0]['Distance']].index - cross_section_group.index[0]
        else:
            bank_indices = river_banks[river_banks['Distance'] > river_center.iloc[0]['Distance']].index - cross_section_group.index[0]

        # Filtrage des pics sur la berge la plus basse
        filtered_peaks_on_lowest_bank = [peak for peak in filtered_peaks if peak in bank_indices]
        print("Pics filtrés sur la berge la plus basse:", filtered_peaks_on_lowest_bank)  # Afficher les pics sur la berge la plus basse

        # Projection des points filtrés sur le profil en travers
        projected_distances.extend(cross_section_group.loc[filtered_peaks_on_lowest_bank, 'Distance'])
        projected_altitudes.extend(cross_section_group.loc[filtered_peaks_on_lowest_bank, 'POINT_Z'])
