...
def export_data(
    df_transects: pd.DataFrame, directory_path: str, output_file_name: str
) -> None:
    """
    ...
    for index, row in df_transects.iterrows():
        ...
        origin_x = df_transects.loc[
            (df_transects["x_sec_id"] == row["x_sec_id"])
            & (df_transects["x_sec_order"] == 0),
            "POINT_X",
        ].iloc[0]
        origin_y = df_transects.loc[
            (df_transects["x_sec_id"] == row["x_sec_id"])
            & (df_transects["x_sec_order"] == 0),
            "POINT_Y",
        ].iloc[0]
        ...
        distance = math.sqrt(
            (row["POINT_X"] - origin_x) ** 2 + (row["POINT_Y"] - origin_y) ** 2
        )
